...
static EBOOL MemCmp(PUB08 Mem1, PUB08 Mem2, BUL32 Len) {
	if (Mem1 == NULL || Mem2 == NULL || Len == NULL)return eFAIL;
	BUL32 Mem1Hash = JsHashApi.JSHash(Mem1, Len);
	BUL32 Mem2Hash = JsHashApi.JSHash(Mem2, Len);
	if (Mem1Hash == Mem2Hash)return eTRUE;
	return eFAIL;
}
